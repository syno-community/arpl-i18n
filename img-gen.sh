...
if [ `ls ../redpill-lkm/output | wc -l` -eq 0 ]; then
  ...
  TAG=`curl -s https://api.github.com/repos/syno-community/redpill-lkm/releases/latest | grep "tag_name" | awk '{print substr($2, 2, length($2)-3)}'`
...
if [ -d ../arpl-addons ]; then
  ...
  TAG=`curl -s https://api.github.com/repos/syno-community/arpl-addons/releases/latest | grep "tag_name" | awk '{print substr($2, 2, length($2)-3)}'`
...
if [ -d ../arpl-modules ]; then
  ...
  TAG=`curl -s https://api.github.com/repos/syno-community/arpl-modules/releases/latest | grep "tag_name" | awk '{print substr($2, 2, length($2)-3)}'`
